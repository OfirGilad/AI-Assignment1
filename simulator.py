...
class TraverseAction:
    ...
    #Here and elsewhere, if needed, break ties by prefering lower-numbered vertices in the x axis and then in the y axis. 
    ...
    def __init__(self,parsed_data):
        ...
        def _handle_a(self, agent):
            ...
            if len(agent["packages"]) == 0 :
                paths = []
                for package in self.state["placed_packages"]:
                   cost , traversePos =  graph.dijkstra(agent["location"][0],agent["location"][1],package["package_at"][0],package["package_at"][1])
